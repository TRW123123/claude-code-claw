...
# These come from the language_server processes running locally.
# Format: (port, csrf_token) — populate from your local Antigravity instance.
# Example discovery: netstat -ano | findstr ":50" + the csrf token from the LS handshake.
ENDPOINTS = [
 # (port, csrf_token),
]

VAULT_DIR = Path(os.environ.get("OBSIDIAN_VAULT_DIR", str(Path.home() / "obsidian-vault")))
OUTPUT_DIR = VAULT_DIR / "antigravity"
CONVERSATIONS_DIR = Path(os.environ.get("ANTIGRAVITY_CONVERSATIONS_DIR", str(Path.home() / ".gemini/antigravity/conversations")))
