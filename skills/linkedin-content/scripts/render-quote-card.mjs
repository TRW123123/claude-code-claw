 ...
 * Persona-spezifisches Branding (Şafak Quiet Authority / Meltem Honest Explorer).
 *
 * Shared zwischen linkedin-content + linkedin-content-meltem.
 *
 * USAGE:
 *   node render-quote-card.mjs --text "..." --persona safak --style quote --out /path/out.png
 *   node render-quote-card.mjs --text "..." --persona meltem --style quote --accent-words "Vertrauen,Mitarbeiter" --out /path/out.png
 *   node render-quote-card.mjs --text "..." --persona safak --style portrait-overlay --portrait public/safak-portrait.jpg --out /path/out.png
 */
...
async function main() {
  ...
  if (!args.text || !args.persona || !args.out) {
    console.error('Usage: render-quote-card.mjs --text "..." --persona <safak|meltem> [--style <quote|portrait-overlay>] [--accent-words "w1,w2"] [--portrait public/path.jpg] --out /full/path/out.png');
    process.exit(2);
  }

  if (!['safak', 'meltem'].includes(args.persona)) {
    console.error(`Invalid persona: ${args.persona}. Allowed: safak | meltem.`);
    process.exit(2);
  }
