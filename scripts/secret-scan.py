...
# Category 1: KNOWN VALUES (specific to this stack — must NEVER leak)
# Literals are split via concatenation so a future git-filter-repo --replace-text
# pass over the historic IDs doesn't break the detection patterns here.
KNOWN_SECRETS = [
    ("Supabase Project-ID (legacy)", "xipqyfmz" + "jdrjvrfbqfhs"),
    ("Google Sheets ID (legacy CRM)", "1vxpDPBuVw" + "waeBZvUDLI-_cLt_pxcXXZpbp-aSWpTlGA"),
    ("Owner private email (gmx)",   r"[Ss]afak\.[Tt]@gmx\.de"),
    ("Owner gmail (legacy)",        "stepe" + "cik88@gmail.com"),
    ("BAFA-Beraternummer pattern",  r"\b\d{6}-\d{3}\b"),  # generic, may have false positives
]
