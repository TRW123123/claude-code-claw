...
## Projekt: APEXX Bau UG
- **Domain:** apexx-bau.de (301-Redirect von `sanper.de`)
- **Repo:** C:/Users/User/Projects/sanper-vision-astro
- **Workspace:** C:/Users/User/Claude/apexx-bau (enthält projekt-spezifische `CLAUDE.md`)
...
- **NEU: Lokales Task-Management (Windows Task Scheduler):** Für systemnahe Aufgaben, die direkten Zugriff auf das lokale Dateisystem benötigen (z.B. Log-Verarbeitung, Wiki-Synchronisation), wird der Windows Task Scheduler genutzt.
  - **`CLAW-Session-Processor` (Windows Task Scheduler, Script: `claw-session-processor.mjs`):** Läuft stündlich, verarbeitet neue Chat-Transkripte und aktualisiert Supabase sowie die Topic-Dateien. Nicht zu verwechseln mit einem `.claude/scheduled-tasks/`-Eintrag.
...
- **AKTUALISIERT: Google Analytics 4 (GA4) Integration:** Der Agent ist mit dem neuen, offiziellen Google Analytics 4 MCP (Machine-Code Protocol) verbunden. Dies ermöglicht direkten, programmatischen Zugriff auf GA4-Daten. Der Live-Zugriff wurde verifiziert; der Agent kann über Tools wie `get_account_summaries` und `run_report` Live-Abfragen für alle verknüpften Properties durchführen, darunter: `Apaydin Automobile` (ID: 519096182), `autoankauf2000.de` (ID: 522915960), `ki-automatisieren.de` (ID: 514601683), `Profilfoto-KI` (ID: 516614685), `PratikYapayZeka` (ID: 494144477), `pratikyapayzeka.co` (ID: 509237122) und `Sanper/APEXX Bau` (ID: 524777115).
...
Ein autonomer Agent erstellt und veröffentlicht Inhalte auf dem persönlichen LinkedIn-Profil von Şafak, gesteuert durch den internen Scheduler von Claude Code.
...
- **Content-Säulen (Türkisch):**
  ...
  - **Freitag (Meinung):** "Was ich denke". Extrahiert persönliche Meinungen, Reaktionen und Learnings von Şafak direkt aus den wöchentlichen CLAW Session Logs.
...
- **Architektur:**
  - Ein zentrales Skript (generate.py — liegt im Repo `C:/Users/User/Documents/Claude/invoices`, nicht unter `C:/Users/User/Claude/scripts`) generiert die PDFs.
...
- **ST-Strategieberatung UG (st-automatisierung.de):** Ein umfassendes SEO-Audit wurde durchgeführt und zahlreiche On-Page- und technische Verbesserungen wurden umgesetzt.
    ...
    - **Datenkorrektur:** Der fälschlicherweise hinterlegte Name "Savaş Topçu" wurde systemweit in "Şafak Tepecik" korrigiert.
...
- **System:** Ein n8n-Workflow (`CLAW – GSC Daily Data Collector v3`) sammelt täglich um 06:00 Uhr Daten aus der Google Search Console für die Domains `ki-automatisieren.de`, `profilfoto-ki.de`, `st-automatisierung.de`, `sanper.de` und `apexx-bau.de`. Die Domain `yapayzekapratik.com` ist aktuell noch nicht integriert.
...
- **Status:** Die Datensammlung für bestehende Projekte läuft fehlerfrei. Ein manueller Backfill wurde durchgeführt, sodass die Daten in Supabase nun eine Historie von ca. 70 Tagen umfassen. Für `apexx-bau.de` wird der erste Datenfluss in 1-2 Tagen erwartet, nachdem die GSC-Property verifiziert wurde.
...
5.  **CLAW-System (Tasks):** Die zwei verbleibenden neuen Scheduled Tasks für die SEO-Automatisierung von `apexx-bau.de` und ein `ai-ugc`-Projekt erstellen.
...
40. **apexx-bau.de (GSC-Monitoring):** Überwachen, ob die GSC-Daten für die neue Domain `apexx-bau.de` in den nächsten 1-2 Tagen korrekt in Supabase ankommen.
41. **apexx-bau.de (Content-Strategie):** Sobald die ersten GSC-Daten verfügbar sind, eine datengetriebene Entscheidung für die nächste Content-Erweiterungsphase treffen.
42. **Rechnungs-Generator (Daten):** Stammdaten (IBAN, Steuernummer) für APEXX Bau UG ergänzen.
...
## Hard Rules
- ~~Der autonome Task Executor (claw-task-executor.mjs) wird die Freigabe haben, SEO-Tasks (Status `pending`) selbstständig zu committen und zu deployen. Code-Änderungen (Status `pending_approval`) werden uncommitted im Repo abgelegt und erfordern manuelle Freigabe via Telegram.~~ (Script entfernt 2026-04-03 — Claude Code ist selbst der Agent, siehe Open-Claw-Migration; Ersatz: Scheduled Tasks + Skills.)
- Der autonome LinkedIn-Agent (Tasks: `linkedin-monday`, `linkedin-wednesday`, `linkedin-friday`) hat die explizite Freigabe, Inhalte direkt und ohne manuelle Freigabe auf dem persönlichen LinkedIn-Profil von Şafak zu veröffentlichen.
...
- Browser-Automatisierung ist für visuelle Website-Audits (Screenshot-Analyse, Layout-Prüfung) mit Chrome-Profil 'Profile 1' (stepecik88@gmail.com) explizit freigegeben und wird aktiv genutzt. Der Agent muss die GSC-URL aus den Browser-Lesezeichen entnehmen und sicherstellen, dass die URL `/u/1/` enthält.
...
- **Infrastruktur-Dateien:** Änderungen an kritischen Konfigurationsdateien (netlify.toml, astro.config.mjs — projekt-lokal pro Astro-Site, nicht unter `C:/Users/User/Claude/scripts`; BaseLayout.astro) erfordern immer eine explizite Bestätigung des Users.
...
- Autonomer SEO-Loop ~~seo-loop-ki-automatisieren~~ (obsolet 2026-04-20 — durch `ki-auto-daily-execution` + `ki-auto-weekly-strategy` ersetzt) war ursprünglich geplant (täglich 09:23, 3-5 Blog-Artikel/Tag, 4-Quellen-Research, Quality Gate, Deploy, Thin-Content Fix alle 3 Tage, Pillar Page bei >25 Artikeln).
- Wöchentlicher Review ~~seo-gsc-weekly-review~~ (obsolet 2026-04-20 — Ersatz für KI-Automatisieren inzwischen im `ki-auto-weekly-strategy` Task integriert; `seo-gsc-weekly-review-st` existiert nur für st-automatisierung) geplant (montags 10:17, GSC-Datenanalyse, Output: seo-weekly-report.md + claw_keyword_inventory.json Updates).
...
- GSC-Analyse zeigt 0 organische Klicks in den letzten 14 Tagen, Positionen im Bereich 70-100, außer Brand-Queries.
- Keyword 'automatisierte datenextraktion' auf Position 37.8 als Striking Distance identifiziert.
- 22 Artikel sind live, KW16 ist noch offen.
- Der 'bafa-berater-Artikel' aus einer früheren Session wurde nicht committed.


### Session Update (2026-04-20)
- Der scheinbare Positions-Drop ist durch einen Ausreißer-Tag am 14.04. verzerrt und keine echte Verschlechterung.
- Die Position liegt konstant bei 55–95.
- Homepage-Impressionen sind von 37 auf 14 gefallen (-62%).
