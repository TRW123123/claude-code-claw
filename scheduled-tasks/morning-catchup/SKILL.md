...
Lies ~/Claude/sessions\agent-log-[HEUTE-DATUM].md (falls vorhanden).
...
1. Lies die SKILL.md des Tasks: ~/.claude/scheduled-tasks\[task-id]\SKILL.md
...
→ Log-Eintrag in ~/Claude/sessions\agent-log-[HEUTE].md: "Catch-up [DATUM] abgeschlossen"
...
3. Führe aus:
 ```bash
 type "$TEMP/tasks.json" | node ~/Claude/scripts/claw-skill-retro.mjs --stdin
 ```
